...
public class BlockEvents implements Listener {
	...
	public BlockEvents(LBRYQuest plugin) {
	...
	void onBlockCatchFire(BlockIgniteEvent event) {
		if (event.getCause().equals(IgniteCause.FLINT_AND_STEEL)) {
			if (event.getPlayer() != null) {
				if (!lbryQuest.canBuild(event.getBlock().getLocation(), event.getPlayer())) {
	...
	void onTrample(PlayerInteractEvent event) {
		...
		if (event.getAction().equals(Action.PHYSICAL)) {
			...
			if (soilBlock.getType() == Material.SOIL) {
				// Check if moderator
				if (!lbryQuest.isModerator(event.getPlayer())) {
					// If the player can't build there cancel it
					if (!lbryQuest.canBuild(soilBlock.getLocation(), event.getPlayer())) {
						event.setCancelled(true);
					} else
	...
	void onBlockBreak(BlockBreakEvent event) {
		...
		} else if (!lbryQuest.canBuild(b.getLocation(), event.getPlayer())) {
			event.setCancelled(true);
		} else {
			event.setCancelled(false);
		}
